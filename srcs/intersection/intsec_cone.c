...
static void intersect_pyramid(t_element *co, t_ray *ray, double tan_teta,
                              double cos_angle) {
    t_coordinates dis;
    double a;
    double b;
    ...
    a = scalar_prod_vec(&ray->direction, &ray->direction) -
        tan_teta * cos_angle * cos_angle;
    b = 2 * (scalar_prod_vec(&ray->direction, &dis) -
             tan_teta * cos_angle * scalar_prod_vec(&dis, &co->vector));
